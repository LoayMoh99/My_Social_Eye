...
class LPQ:
    def __init__(self, win_size=11, ALPHA=11) -> None:
    ...
    def compute(self, image):
        filterResp1 = convolve2d(convolve2d(
            image, self.w0.T, 'valid'), self.w1, 'valid')
        filterResp2 = convolve2d(convolve2d(
            image, self.w1.T, 'valid'), self.w0, 'valid')
        filterResp3 = convolve2d(convolve2d(
            image, self.w1.T, 'valid'), self.w1, 'valid')
        filterResp4 = convolve2d(convolve2d(
            image, self.w1.T, 'valid'), self.w2, 'valid')
        ...
        return np.histogram(LPQdesc.flatten(), range(256))[0]
...
class LPQPlus:
    ...
    def compute(self, image):
        ...
        filterResp1: np.ndarray = convolve2d(convolve2d(
            image, self.w0.T, 'valid'), self.w1, 'valid')  # (0, a)
        filterResp2: np.ndarray = convolve2d(convolve2d(
            image, self.w1.T, 'valid'), self.w0, 'valid')  # (a, 0)
        filterResp3: np.ndarray = convolve2d(convolve2d(
            image, self.w1.T, 'valid'), self.w1, 'valid')  # (a, a)
        filterResp4: np.ndarray = convolve2d(convolve2d(
            image, self.w1.T, 'valid'), self.w2, 'valid')  # (a, -a)
        ...
        vote1 = np.array(list(map(lambda theta: (
            np.cos(theta1 - theta)**self.VOTE_ALPHA).sum(), self.ANGLES)))
        vote2 = np.array(list(map(lambda theta: (
            np.cos(theta2 - theta)**self.VOTE_ALPHA).sum(), self.ANGLES)))
        vote3 = np.array(list(map(lambda theta: (
            np.cos(theta3 - theta)**self.VOTE_ALPHA).sum(), self.ANGLES)))
        vote4 = np.array(list(map(lambda theta: (
            np.cos(theta4 - theta)**self.VOTE_ALPHA).sum(), self.ANGLES)))
...
def lpq_plus(image, win_size=11, q_levels=8):
    ...
    # Applying filters and getting response
    filterResp1: np.ndarray = convolve2d(convolve2d(
        image, w0.T, 'valid'), w1, 'valid')  # (0, a)
    filterResp2: np.ndarray = convolve2d(convolve2d(
        image, w1.T, 'valid'), w0, 'valid')  # (a, 0)
    filterResp3: np.ndarray = convolve2d(convolve2d(
        image, w1.T, 'valid'), w1, 'valid')  # (a, a)
    filterResp4: np.ndarray = convolve2d(convolve2d(
        image, w1.T, 'valid'), w2, 'valid')  # (a, -a)
    ...
    vote1 = np.array(
        list(map(lambda theta: (np.cos(theta1 - theta)**VOTE_ALPHA).sum(), ANGLES)))
    vote2 = np.array(
        list(map(lambda theta: (np.cos(theta2 - theta)**VOTE_ALPHA).sum(), ANGLES)))
    vote3 = np.array(
        list(map(lambda theta: (np.cos(theta3 - theta)**VOTE_ALPHA).sum(), ANGLES)))
    vote4 = np.array(
        list(map(lambda theta: (np.cos(theta4 - theta)**VOTE_ALPHA).sum(), ANGLES)))
    lpq_desc = np.concatenate((vote1, vote2, vote3, vote4))
